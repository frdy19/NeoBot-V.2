                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            ...
                            const image = await new knights.Welcome().setUsername(nameusr).setGuildName(gcname).setGuildIcon(ppgc).setMemberCount(memcount).setAvatar(ppmem).setBackground("https://telegra.ph/file/32b632e34e1483147d8b3.jpg").toAttachment();
                            let wel = image.toBuffer();
                            const imagelea = await new knights.Goodbye().setUsername(nameusr).setGuildName(gcname).setGuildIcon(ppgc).setMemberCount(memcount).setAvatar(ppmem).setBackground("https://telegra.ph/file/32b632e34e1483147d8b3.jpg").toAttachment();
                            let lea = imagelea.toBuffer();
                    ...
                    this.sendButton(id, text, 'Welcome/Bye By Arifzyn', action === 'add' ? wel : lea, [
                        [action == 'add' ? 'Welcome' : 'Sayonara', action === 'add' ? '.intro' : 'Arifzyn '],
                        ['Menu', '.menu']
                        ], fkontak, { mentions:[user] })
...
let prm = {
  ...
  if (prm) return conn.reply(m.chat, prm, m)
